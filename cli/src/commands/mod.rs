...
pub fn matching(args: Args)-> Result<(), &'static str>{
    match args.command{
        ...
        Command::Ndvi{default,area, from, till, hls, landsat}=>{
            ...
            if default {
                ...
                ndi::run("http://localhost:3000/ndvi", Some("".to_string()),&from, &till, "NDVI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndvi", Some(area), &from, &till,"NDVI","TARGET",Some("".to_string()), &source)
            }
        }
        Command::Ndci {default,area, from, till, hls}=>{
            let source = get_hls_source(hls);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndci", Some("".to_string()),&from, &till, "NDCI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndci", Some(area), &from, &till,"NDCI","TARGET",Some("".to_string()), &source)
            }
        }
        Command::Ndti { default,area, from, till, hls, landsat}=>{
            let source = get_dataset_source(hls, landsat);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndti", Some("".to_string()),&from, &till, "NDTI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndti", Some(area), &from, &till,"NDTI","TARGET",Some("".to_string()), &source)
            }
        }
        ...
        Command::Ndwi { default,area, from, till, hls, landsat }=>{
            let source = get_dataset_source(hls, landsat);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndwi", Some("".to_string()),&from, &till, "NDWI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndwi", Some(area), &from, &till,"NDWI","TARGET",Some("".to_string()), &source)
            }
        }
        Command::Ndmi { default,area, from, till, hls, landsat }=>{
            let source = get_dataset_source(hls, landsat);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndmi", Some("".to_string()),&from, &till, "NDMI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndmi", Some(area), &from, &till,"NDMI","TARGET",Some("".to_string()), &source)
            }
        }
        Command::Ndbi { default,area, from, till, hls, landsat }=>{
            let source = get_dataset_source(hls, landsat);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndbi", Some("".to_string()),&from, &till, "NDBI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndbi", Some(area), &from, &till,"NDBI","TARGET",Some("".to_string()), &source)
            }
        }
        Command::Ndsi { default,area, from, till, hls, landsat }=>{
            let source = get_dataset_source(hls, landsat);
            if default {
                let api_key = get_api_key().map_err(|_| "Failed to get API key")?;
                ndi::run("http://localhost:3000/ndsi", Some("".to_string()),&from, &till, "NDSI","DEFAULT",Some(api_key), &source)
            } else {
                let area = area.ok_or("Provide --area or use --default")?;
                ndi::run("http://localhost:3000/ndsi", Some(area), &from, &till,"NDSI","TARGET",Some("".to_string()), &source)
            }
        } 
    }
}
